...
职责：
    1. 工具容器：注册 / 注销 / 查询（原 ToolRegistry 职责合并于此）
    2. 工具执行：execute（工具级并发信号量 + 超时保护 + 自动重试）
    3. 执行统计：调用次数 / 成功率 / 平均耗时
    4. 钩子机制：工具执行前后扩展
...
是 Agent 维度（GPU/服务器资源），而非 LLM API 维度（RPM/TPM 由 reservation_limiter 覆盖）。
"""
