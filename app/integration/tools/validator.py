...
- 用固定 Draft 2020-12 校验器收集全部错误，一次反馈给 LLM
  （比 validate() 选择一个代表错误更利于下一轮全部修正，减少归因往返）
...
class ParameterValidator:
    ...
    def validate(
        ...
        try:
            ...
            # 本地根引用必须指向收紧后的 Schema；evolve 会沿用原解析器。
            if effective is not schema:
    ...
    def _with_reject_unknown(self, schema: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self._reject_unknown or schema.get("additionalProperties") is False:
            return schema
        wrapped = dict(schema)
