...
背景（2026-09-21 定位）：`tests/unit/test_reflection.py` 的严格期限用例原先给 0.1 秒总
预算，而该窗口内要跑完真实 ReAct 草稿阶段与工具执行。观察发现**一次外部插件目录扫描
就吃掉内部期限的绝大部分**，工具本体耗时接近 0；余量随机器负载浮动，全量套件的 CPU /
磁盘争用一上来就越界，先命中的是工具自身 deadline（抛 ToolDeadlineExceededError），
用例根本到不了它要断言的自查迟到阶段。
