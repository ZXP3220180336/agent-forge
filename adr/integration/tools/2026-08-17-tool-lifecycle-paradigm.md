...
> **后继条款（2026-09-13）**：[TOOLS-ADR-008 D5–D8](2026-09-13-tool-execution-lifecycle.md#d5-调用后事实与并行提交)保留连接池与生命周期钩子选择，补充在途执行、迟回结果、卸载及持久接管责任。本文 Context 中“无状态→可重试/幂等”不足以证明远端副作用可重复，重试依据由后继 D3 收紧。后继方向已接受并大部分落地（2026-09-21 核验）：在途执行跟踪、迟回值 Owner 与卸载保护已随 Piece④⑤ 完成（`ToolExecutionSupervisor.owns` / `retain_record` / loader 在途拒绝卸载）。**持久接管仍未实现**，归 TOOLS-ADR-008 的 Piece⑥。
