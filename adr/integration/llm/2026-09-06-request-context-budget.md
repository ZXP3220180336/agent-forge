...
8. 取消与总时长由策略层确定业务 `cancel_event` 和 monotonic 绝对 `deadline`，Integration 的 `generate` / `async_generate` 将二者贯穿每次预算准入、reserve 排队、SDK create、retry / 整流 / 续接退避及流读取；`generate_structured` 把同一信号透传给每笔子调用，并在最外层收敛终止。执行终止使用私有类型化信号，在 Facade 边界翻译为 shared 的 `LLMCancelledError` / `LLMDeadlineExceededError`，不得用可被归类为网络超时的内置 `TimeoutError`。流式业务取消在必要 close/结算后统一抛 `LLMCancelledError`，Integration 不生成取消 SSE、不写 `StreamResult.error`；终止前已取得的内容与 usage 留在调用方持有的 `StreamResult`，由领域编排独占取消终态提交。外部 task 硬取消仍原样传播 `asyncio.CancelledError`。reserve 的可中断等待由 `execution_control.py` 外置包装，`ReservationLimiter` 不直接依赖业务取消。成本预算仍由策略层护栏管理（实施细化见 [LLM-050](../../../issues/integration/llm/2026-09-13-stream-business-cancellation-contract.md)）。
