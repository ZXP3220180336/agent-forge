...
- [EmbeddingService 文本向量化说明文档](#embeddingservice-文本向量化说明文档)
  - [📋 目录](#-目录)
  - [模块概述](#模块概述)
    - [定位与职责](#定位与职责)
    - [与其它服务的关系](#与其它服务的关系)
...
vector = await container.embedding_service.embed("Hello", model="text-embedding-3-small")

# 缓存操作
container.embedding_service.clear_cache()
print(container.embedding_service.cache_size)
```
...
> `Container.initialize()` 用 `ClientManager.get_client("main")` 作为 client——即嵌入请求与主模型共享连接池实例。`max_batch_size` / `enable_cache` 当前无配置项，为代码内默认值（20 / True）。

---

## 相关文档

- [集成层总览](../README.md)（EmbeddingService 的定位）
- [LLM 层说明](../llm_doc/llm.md)（ClientManager 连接池、嵌入配置分组）
- [记忆系统（预留）](../../domain_doc/memory_doc/memory.md)（规划中「长期记忆向量化」的能力来源）
