...
**预留公式**（[llm_service.py](../../../app/integration/llm/llm_service.py) 的 `_count_prompt_tokens`）：`estimated = prompt_tokens + max_tokens`（输出上限的保守估算），TPM 桶按此扣减。**这个数是「上限」而非「实际」**——实际输出往往远小于 max_tokens。
