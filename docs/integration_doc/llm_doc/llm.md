...
- **非流式配额结算兜底（LLM-002，2026-08-16）**：`generate()` 解析 + 结算阶段放 `try/finally`——成功路径 `settle(actual)` 退 TPM 差；解析抛异常 → `settle(None)` 保留配额、settle 被取消 → `cancel()` 兜底 + re-raise，与流式 `rectified_stream` finally 对称（修复非流式配额泄漏）。详见 [问题文档](../../issues/llm/llm-002-generate-quota-settle-fallback.md)
