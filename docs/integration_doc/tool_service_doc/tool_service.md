...
ToolService（统一入口：execute / get_openai_tools / get_stats ...）
        │
        └──────► app/integration/tools/builtin/（内置工具实现）
...
### 内置工具清单（`app/integration/tools/builtin/`）
...
- 均委托 `BaseTool.to_openai_tool()` / `to_openai_response()`，数据源为工具的 `name` / `description` / `parameters`（见 [工具层文档](../../integration_doc/tools_doc/tools.md)）
...
- `validate_parameters` 默认实现只做「异常参数」与「必填缺失」检查（见 [base.py](../../../app/integration/tools/base.py)）
...
- [工具层说明](../../integration_doc/tools_doc/tools.md)（BaseTool / ToolResult / 内置工具）
