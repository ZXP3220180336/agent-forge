...
2. **审计留痕**：每次工具调用记录一条结构化事件（`tool_call`），覆盖成功 / 失败 / 未注册 / 校验失败 / 超时 / 审批拒绝 / 准入拒绝全路径（cancel / deadline / run_stop 的类型化控制异常不写审计，由上层终态承接）
...
| 触发 | 仅成功路径 | 成功 / 失败 / 未注册 / 校验失败 / 超时 / 审批拒绝 / 准入拒绝全路径 |
