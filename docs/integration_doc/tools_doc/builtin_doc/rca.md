...
3. **证据链 metadata**：每个工具结果带数据来源 `source` / 查询键 / 时间戳（`search_historical_rca` 以 `top_confidence` 替代时间戳——metadata 为 `source` / `query` / `top_k` / `top_confidence`；其余 4 工具带 `timestamp` 锚点 = 结果集**最大时间**，与数据顺序无关）——产品「证据链」亮点的载体
...
`tests/unit/test_rca_tools.py`（22 用例）：各工具正常查询 + 证据链 metadata / 参数校验 / 校验错误不回显值 / 业务未找到 / **时间锚点取最大**（与数据顺序无关）/ **窗口空结果归因区分** / **FDC 阈值契约**（≥5% 判定 + query 派生 status）/ LOT-A123 根因故事（骤降、FDC 偏离、center_cluster、历史命中）/ 数据可复现 / init_default_tools 装配 10 工具 / 时间窗口过滤（P0-1：FDC 偏离发展、yield 过滤、单侧缺省、end 短格式、`_in_range` 单侧纯时间与混合窗口）/ 相关度置信度（P0-2）。
...
- [TOOLS-035 问题记录](../../../../issues/integration/tools/2026-08-20-rca-b100-timeline.md)（对照批次解耦）
- [TOOLS-036 问题记录](../../../../issues/integration/tools/2026-08-20-rca-fdc-threshold.md)（FDC 阈值契约化）
