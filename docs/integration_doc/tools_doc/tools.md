...
5. **外部工具惰性检查**：`execute` 入口对比 external 目录签名，变化才重扫（无常驻扫描，刷新任务有 Owner；在途版本的卸载延后）

### 设计启示（来自 Agent 行为实验）

> 依据：learning-agent 项目的工具选择实验（同一温度转换任务 × 4 组对照，验证「schema 质量 → Agent 选择准确性」与「Agent 盲试行为」）。此处只留工业级结论，实验细节见该学习项目。

**三层语义信号**：模型判断「用哪个工具」时，按强度消费 schema 的三个字段——**工具名 > 描述 > 参数名**：

| 信号 | 例 | 实验结论 |
| --- | --- | --- |
| ① 工具名 | `celsius_to_fahrenheit` | 名字本身携带方向语义，描述模糊也选对 |
| ② 描述 | "把摄氏温度转为华氏度。公式 F = C*9/5+32" | 名字无意义时，带方向词的描述仍能救场 |
| ③ 参数名 | `celsius` / `fahrenheit` | 名字、描述都无意义时，参数名仍泄露方向 |

→ 写工具 schema 时三个字段都要刻意设计：缩写名、省略示例的描述、无意义参数名（`value1`）都会削弱 Agent 的选择准确性。**很多工具误用故障的根因是 schema 含糊，而非模型笨。**

**Agent 盲试行为**：三层信号全关时，Agent 不会「猜错就结束」，而是**两个相似工具都调用、靠输出反推哪个正确**——最终答案对，但代价是「全试」。若工具有副作用（写/删/执行/外部调用），「全试」是灾难。本模块的防御机制正是对这一真实行为的回应：风险分级 L0-L3 标注危险度（[security.md](security.md)）、审批 ApprovalGate 阻断危险调用、审计 ToolAuditor 全程留痕、jsonschema 严格校验减少参数试错（[validator.md](validator.md)）。
...
正式执行及两种模型导出受[能力启用门禁](execution.md#当前启用边界)约束；注册成功不表示获准执行。当前写入/未知效果/强制审计工具保持注册但隐藏，直接点名执行返回 REJECTED。
...
`ErrorCode`（[app/domain/ports/tool_gateway.py](../../../app/domain/ports/tool_gateway.py)）系统级 7 码：`NOT_REGISTERED`（未注册）/ `JSON_PARSE`（参数 JSON 解析失败）/ `VALIDATION`（校验失败）/ `REJECTED`（能力未启用或审批拒绝）/ `CAPACITY_EXCEEDED`（共享准入容量或等待队列已满，工具未执行且不重试）/ `TIMEOUT`（执行超时）/ `UNKNOWN`（未捕获异常）。工具业务错误为 `None`（`error` 字符串承载 LLM 归因）——**错误码 + 中文归因并存**：错误码供审计聚合与证据链可审计性，`error` 供 LLM 修正。
...
| `execute(**kwargs) -> ToolResult` | 抽象；独立适配器业务入口，不能替代正式 ToolService 准入；业务错误走返回值 |
| `invoke(parameters, execution)` | 受控执行入口；线程工作必须通过 execution.run_sync 登记完整同步操作 |
| `describe_execution(parameters)` | 无副作用的可信能力声明；缺省 UNKNOWN，当前启用条件见 execution 文档；空参数无法判定时不导出 |
...
| 真实执行接管 | execution.py | 真实任务/线程生命周期、有界清理与迟回值归属 | [execution.md](execution.md) |
...
`builtin/` 自动发现 `BaseTool` 子类（无需注册代码）。10 个内置工具及其注册名（注册清单不等于当前启用清单）：

| 工具 | 注册名 | 职责 |
| --- | --- | --- |
| SearchTool | `search` | 网络搜索（Tavily） |
| ReadFileTool | `readFile` | 读取文件 |
| WriteFileTool | `writeFile` | 写入文件；等待 B，当前禁止正式执行 |
| CodeExecTool | `code_exec` | 终端命令执行；等待 B，当前禁止正式执行 |
