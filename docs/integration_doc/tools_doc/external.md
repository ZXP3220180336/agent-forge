...
2. **调用触发、TTL 限频刷新**：无后台任务或后台轮询；TTL 内复用签名，到期后的下一次工具调用触发磁盘检查，变更才重扫
...
`ToolService.execute` 入口先调 `maybe_refresh()`：**TTL（`_DIR_SIGNATURE_TTL` = 1s）内零磁盘 IO**（复用上次签名结果），到期后对比**目录签名**（文件集 + 各文件 mtime/size，经 `asyncio.to_thread` 执行不阻塞事件循环）与上次扫描值，**签名无变化则不重扫**，变化才重扫；TTL 到期后的签名检查本身仍有磁盘工作。因此：

- 外部工具文件新增 / 修改 / 删除后，由 TTL 到期后的下一次工具调用检查并加载变更；没有后续调用就不会主动刷新。1s 是签名检查的缓存间隔，不是无条件的变更生效时限
