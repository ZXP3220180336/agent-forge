...
```
AppState.initialize() / LLMService / 外部调用方
        │
        ▼
...
**`_pending_closes`**：无运行事件循环时（纯注册阶段，如 `AppState.initialize` 之前），`register_config` 无法 fire-and-forget 关闭旧 client，登记到此列表由 `close_all` 统一关闭——不再静默忽略，可追踪。
...
`register_config` 的参数（来自 `AppState.initialize()` 读 settings 后调用）：
