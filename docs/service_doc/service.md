...
> **更新日期**：2026-08-03
> **文档定位**：服务层（`app/services/`）的模块说明，覆盖模块概述、实现状态、各模块核心功能与使用示例。LLM 子包细节见 [LLM 层文档](llm_doc/llm.md)。
...
```
app/services/
├── __init__.py                 ← 包入口，导出 6 个已实现服务类
├── session_manager.py          ← SessionManager 会话管理（Redis 热缓存 + DB 持久化）
├── context_manager.py          ← ContextManager 上下文组装 / token 截断
├── tool_service.py             ← ToolService + ToolStats 工具统一入口
...
1. **单例装配**：`app_state` 持有全部服务实例，启动时经 `AppState.initialize()` 统一初始化，关闭时统一清理；依赖注入 `get_tool_service` 等返回同一实例
...
API 层（chat / session 路由）
  ...
  │   └── ToolService ──► app/tools/ ──► 执行 │
  │                                           │
  │  EmbeddingService ──► OpenAI /embeddings  │
  └───────────────────────────────────────────┘
        │
        ▼
  基础设施层（Redis / Database，由 app_state 直接管理）
```
...
# 存取消息
await app_state.session_manager.add_message(session_id, "user", "分析这批不良率", token_count=42)
...
async def handle_chat(user_input, messages, context, agent):
    # 并发超限时在此等待
    async for event in app_state.task_service.run_agent(
...
**文件**：`app/services/memory_service.py`（❌ 空文件，未实现）
...
- 文档占位：`docs/core_doc/memory_doc/` 目录已建但无内容
...
| `llm_*` | 见 [LLM 文档](llm_doc/llm.md) | LLMService / EmbeddingService | 模型 / 重试 / 熔断 / 限流等 |
...
- [LLM 层说明](llm_doc/llm.md)（LLMService 底层 llm/ 子包详解）
- [Task 模块](task_doc/task.md)（任务调度顶层计划）
- [Session 模块](session_doc/session.md)（会话管理详解）
- [Context 模块](context_doc/context.md)（上下文管理详解）
- [ToolService 模块](tool_service_doc/tool_service.md)（工具服务详解）
- [Embedding 模块](embedding_doc/embedding.md)（向量化详解）
