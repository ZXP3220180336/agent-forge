...
## 2026-09-10 工具协议修正预算（REASON-017）

- **总循环上限不能替代局部恢复预算**：`max_iterations` 只保证最终停止；finish_reason 不一致、final_answer 校验失败和工具参数 JSON 失败都会再次产生付费请求，必须有更小的独立上限。
- **共享计数按恢复目标划分，不按代码分支划分**：三类错误都在修正同一个“工具调用协议”，分别计数会允许模型在错误类型间切换并反复重置。合法工具协议才是恢复成功信号；LLM 失败或空输出不能清零。
- **协议校验先于历史所有权转移**：无效 assistant/tool_calls 若先写入历史，下一轮会携带未配对工具消息，把可恢复的模型错误放大为 provider 400。先校验、后持久化是消息协议的事务边界。
- **名称特判必须受真实模式约束**：只有启用 output_schema 时，`final_answer` 才是终止工具；按名称全局排除会让该调用的参数不参与指纹，整轮仅该调用时指纹恒定，参数各异的连续轮次被误判为同一动作（过严，不豁免也不放行）。
- **外部可调总闸必须有边界且不遮蔽部署配置**：请求字段用可选值表达“未覆盖”，由装配根提供生产默认；对外覆盖值必须复用 settings 的 1..100 约束。

## 2026-09-10 跨策略执行护栏（REASON-016）

- **守卫结果必须是领域类型，展示字符串不能承担控制流协议**：字符串二元组无法稳定表达取消、deadline、成本和上下文超限的优先级，也迫使每个策略重复推断。复用 `AgentErrorKind` 的不可变结果对象后，优先级与短路行为可以直接测试。
- **付费调用采用“调用前准入，成功后先接管再复查”**：只做调用前检查会漏掉 await 期间发生的取消和超限；调用后先检查又会丢失已经返回的结果与 usage。正确顺序是接管内容、usage 和资源所有权，再按固定优先级终止后续副作用。
- **模型意图不等于用户可见成果**：尚未执行的 `tool_calls` 不能单独让当前轮覆盖上一轮正文。终态选择应以已经可交付的 content/reasoning 为依据，工具记录以实际执行结果为准。
- **辅助函数的单测不能替代真实状态迁移测试**：即使 `_terminal_result()` 已正确忽略未执行 `tool_calls`，主循环仍可能在正常返回路径无条件覆盖最近成果，或在调用后护栏中直接绕过辅助函数。涉及跨轮所有权时，测试必须覆盖“上一轮可见成果 → 当前轮仅工具意图 → 终态触发”的完整路径。
- **注释中的仲裁模型必须服从真实分发契约**：`dispatch_error()` 在 RAISE 时立即抛异常，调用方就不可能再收集 RAISE 决策做二次仲裁。保留不可达分支和“全部收集后统一仲裁”的说明会误导后续修改；应准确记录“RAISE 立即传播，否则 STOP > CONTINUE”。
