...
| 在信任边界收敛外部返回值 | 动态加载的适配器只受类型标注约束，而标注不是运行时约束。只判「是不是契约对象」不判字段，字段级越界会继续逃逸：`content=None` 让 ReAct 切片抛 `TypeError`、`error_code="TIMEOUT"` 让 `.value` 抛 `AttributeError`，都中断整批工具调用；非布尔 `success` 更隐蔽——不抛异常，却被真值判定静默当成业务成功。检查范围应由**下游如何消费该字段**决定（切片 / 取 `.value` → 抛异常；真值判定 → 静默错结论；转交事实与审计 → 必须是声明类型），而不是由“会不会抛”决定；也不要用静默强转兜底，那会把适配器缺陷变成看起来正常的观测回喂给模型。 | 2026-09-19 非法返回值收敛；[executor 文档](integration_doc/tools_doc/executor.md)、[TOOLS-ADR-008](../adr/integration/tools/2026-09-13-tool-execution-lifecycle.md)。 |
