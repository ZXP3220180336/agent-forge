...
| 统一策略 Guard 或调整调用后顺序 | 类型化 Guard 改善了优先级表达，但机械地把所有返回路径都改成先 Guard，会覆盖 Reflection 的 critique 通过与轮次上限早退。通用检查点必须对照策略契约。 | [REASON-016](../issues/domain/reasoning/2026-09-10-cross-strategy-guard-priority.md)、[REASON-020](../issues/domain/reasoning/2026-09-11-reflection-guard-checkpoint.md)；[Reflection 例外](engineering/agent-runtime-rules.md#reflection-exception)。 |
...
| create 已调度而终止、或退款被取消打断 | 请求是否实际开始不能靠调度先后猜测；重复退款和过度退款可能虚增可用配额。 | [LLM-042](../issues/integration/llm/2026-09-07-reserve-r5-cancel-interrupted-rpm-leak.md)、[LLM-044](../issues/integration/llm/2026-09-08-execution-control-through-every-call.md)；[请求规范](engineering/integration-rules.md)。 |
...
| 扩展 handler、日志或审计影响业务收尾 | handler 自身失败曾击穿 UNKNOWN 兜底；但不能据此宣称所有日志路径已经隔离。非关键观测治理目标与现行传播差异仍须回仓核验。 | [SHARED-001](../issues/shared/error_handling/2026-08-30-handler-exception-defense.md)、[REASON-014](../issues/domain/reasoning/2026-09-09-internal-timeout-misclassified-as-deadline.md)、[单一来源 ADR](../adr/2026-09-12-single-source-governance.md)；[G0](engineering/ai-engineering-rules.md#g0)。 |
