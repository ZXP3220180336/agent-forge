...
推理策略模块是核心层的**策略库**，为 Agent 提供多种推理方式的实现。当前 Agent 的 ReAct 策略直接实现在 `app/core/agent/executor.py` 中；本模块规划将这些推理方式**抽离为独立策略**，未来可作为 `_strategy_cycle()` 的替代实现。
