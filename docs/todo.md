...
**授权边界**：用户已确认六项方向并授权实施 P0 规格工作；本轮修改 ADR、todo、配置和部署文档，不创建运行类、不执行数据库迁移、不提交 Git。P0-A/B 规格已形成，供实施前评审；P1～P5 不是本轮自动执行授权。
...
2026-09-14 最新结论：P0 规格静态复核完成，身份归属、未知状态、幂等比较、核验计数和跨层职责的歧义已修正，P3/P4 端口与消费方须同批迁移；未发现阻塞下一阶段的明确设计矛盾。六项方向不变，P1～P5 仍待实施授权。该次复核的 alignment、diff 及四份文档 121 个本地链接/锚点检查通过；未运行功能测试或真实数据库/宿主实验，运行保证仍待实施验收。
