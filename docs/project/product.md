# 产品定位与方向（产品决策文档）

> **方向决策时间**：2026-08-03；维护日期：2026-09-13
> **文档定位**：产品定位、方向取舍、用户价值与可演示里程碑的正式维护入口。模块实现状态只在 [ALIGNMENT.md](../ALIGNMENT.md) 登记；本文的产品目标不表示已经完成代码或真实生产验证。
> **核心表述**：本项目 = **多 Agent 任务执行引擎** + 具体驱动场景。引擎是能力核心，场景决定工具与演示。

---

## 📋 目录

- [产品定位与方向（产品决策文档）](#产品定位与方向产品决策文档)
  - [📋 目录](#-目录)
  - [一句话定位](#一句话定位)
  - [产品导向：所有设计、实现与文档决策的第一判断](#产品导向所有设计实现与文档决策的第一判断)
  - [方向决策记录](#方向决策记录)
  - [主方向：良率根因分析](#主方向良率根因分析)
    - [场景定义](#场景定义)
    - [良率 RCA 的业务对象与报告生命周期](#良率-rca-的业务对象与报告生命周期)
    - [行业背景（方向决策时的外部参照）](#行业背景方向决策时的外部参照)
    - [为什么契合引擎](#为什么契合引擎)
    - [工程亮点（区别于通用 demo）](#工程亮点区别于通用-demo)
    - [落地方式（不依赖真实 fab）](#落地方式不依赖真实-fab)
    - [工具清单](#工具清单)
    - [按需求评估的扩展能力](#按需求评估的扩展能力)
  - [备选方向：工艺知识助手](#备选方向工艺知识助手)
  - [已关闭方向：EDA 设计辅助](#已关闭方向eda-设计辅助)
  - [里程碑与闭环](#里程碑与闭环)
    - [验收标准](#验收标准)
  - [工具模块产品导向取舍](#工具模块产品导向取舍)
    - [能力评估](#能力评估)
    - [证据与投入边界](#证据与投入边界)
    - [取舍](#取舍)
  - [相关文档](#相关文档)

---

## 一句话定位

**多 Agent 任务执行引擎**：用户提交目标 → 系统异步受理 → 主 Agent 拆分 → 并行子 Agent 执行 → 汇总交付。当前驱动场景为**半导体良率异常根因分析（Yield RCA）**。

> 引擎与场景的关系：引擎（队列/调度/编排/状态）是长期资产，场景（工具/数据/提示词）是短期外壳。外壳可换，引擎沉淀。

---

## 产品导向：所有设计、实现与文档决策的第一判断

本项目为多 Agent 任务执行引擎，驱动场景为半导体良率异常根因分析（Yield RCA）。产品主链路：良率工程师提交某批次良率异常 → 系统异步受理 → 主 Agent 拆分 → 并行子 Agent 排查 → 输出带证据链的根因报告。

一切设计、实现和文档决策先判断对产品主链路的真实价值，再进入工程 Gate。技术能力只有服务产品目标时才值得建设，不为纯技术优雅或前瞻假设增加成本；产品价值不豁免 G0 正确性与安全约束。

设计每个功能、模块或组件时，依次回答：

1. 服务谁：对应哪个真实场景、哪类用户（良率工程师）？
2. 价值几何：是否推进拆分、并行排查和证据链报告，价值直接还是间接？
3. 成本匹配：实现和维护成本是否与价值匹配，能否简化而不损害价值？
4. 不做或预留：当前不服务主链路的能力默认不做，或仅在 ADR 记录升级路径，出现真实需求后再落地。

## 方向决策记录

| 方向 | 决策 | 原因 |
| --- | --- | --- |
| 多 Agent 任务执行引擎（核心能力） | ✅ 认同 | 引擎为长期资产；已有模块与未完能力按 ALIGNMENT 核验，task.md 规划不等于已实现基建 |
| 良率根因分析（Yield RCA，驱动场景） | ✅ 主方向 | 行业推进最热、与引擎契合度最高、可落地（模拟数据） |
| 工艺知识助手（工业 RAG） | 🔶 第二项目备选 | 与引擎关联弱（单 Agent 检索），但工程价值独立，作为后续项目 |
| EDA 设计辅助 | ❌ 关闭 | 结构性不可落地（见下文详细原因） |
| 通用调研/报告 | ❌ 弃用 | 无行业纵深，仅作技术验证外壳 |
| 智能客服 / 知识库 RAG | ❌ 弃用 | 同质化严重，与多 Agent 编排关联弱 |

---

## 主方向：良率根因分析

### 场景定义

良率工程师提交"某批次良率骤降（如 95%→82%），排查根因"，系统异步受理 → 主 Agent 拆分 → 子 Agent 并行排查 → 汇总带**证据链**的根因报告。

### 良率 RCA 的业务对象与报告生命周期

**良率**是一个批次或晶圆中合格芯片占总芯片的比例。良率突然下降通常只是症状，不能直接说明原因。根因分析（Root Cause Analysis，RCA）的工作，是围绕同一批次、设备和时间窗口，把良率趋势、设备告警与保养记录、FDC 工艺参数、晶圆缺陷图和历史案例交叉核对，形成可验证的原因假设，并明确还缺少哪些数据。

本产品处理一次良率异常时，业务信息依次形成以下对象：

| 业务对象 | 含义 | 为什么必须保留 |
| --- | --- | --- |
| **证据记录** | 一次工具查询及其来源、参数、时间、成功状态、结果或错误。 | 说明事实从哪里来，支持复核；失败记录也能说明某项数据尚未取得，但不能充当支持结论的证据。 |
| **证据链** | 围绕同一异常串联的多条证据记录，例如良率下降时间、设备告警和同窗口参数漂移。 | 让工程师区分“观测到的事实”和“模型作出的推断”，并沿来源重放排查过程。 |
| **候选稿（draft）** | 根据当前证据形成的阶段性 RCA 报告，包含摘要、候选根因、支持证据、置信度、显式放弃和下一步建议。 | 它是待审查的工作成果，不等同于已经确认的最终根因。后续自查必须围绕这份稿件验证每个主张。 |
| **问题清单（issues）** | 自查发现的缺陷，例如结论缺少证据、数据互相冲突、引用不存在、置信度过高或仍有关键证据缺口。 | 指导下一轮修订。严重问题优先于措辞等次要问题，否则报告可能在证据不足时给出过强结论。 |
| **可提交报告** | 问题消除或达到执行边界后，对最新候选稿作出的提交结果。 | 对外必须同时呈现结论、证据、置信度与未决项；证据不足时应明确降低结论强度或放弃判断。 |

Reflection 的业务循环是“候选稿 → 证据审查 → 问题清单 → 修订稿 → 再审查”。审查关注的核心不是文字是否流畅，而是每项结论能否由现有证据支持、不同来源是否一致、置信度是否与证据强度匹配。循环在审查通过、达到迭代上限，或取消、期限、成本和上下文等运行护栏终止时结束；任何终止都不能抹除已经取得的证据和最近一份完整候选稿。

例如，某批次良率从 95% 降到 82%，同一时间窗口内设备出现温控告警，FDC 显示温度偏移 3 °C。三项观测均来自证据记录；“温控漂移可能导致良率下降”是候选根因；若缺少受影响晶圆的缺陷图或对照批次，问题清单应指出证据缺口，报告只能给出中低置信度判断并建议补查，不能把相关性写成已证实因果关系。

一次排查可能累积大量日志、量测点和历史案例，超过模型单次可读的上下文。此时系统缩减的是**本轮送审视图**，不是原始业务记录：原始证据、完整候选稿和完整问题清单仍由各自责任方保存。送审视图优先保留当前稿件已引用的证据、时间和量测锚点、结论与显式放弃，以及严重和较新的问题；被省略内容必须有可计数标记，避免模型误以为输入完整。若连最小可审查骨架都无法容纳，则停止新的模型调用并返回最近可验证成果。

### 行业背景（方向决策时的外部参照）

> 下列数字与行业判断是方向讨论时收集的外部材料，本轮未重新核验。它们不构成本项目性能承诺或效果证明；对外引用和更新产品决策前核查原始来源、时间与适用范围。

- 麦肯锡：半导体制造 AI/ML 价值约 **40%** 集中在良率/质量类场景
- [晶合集成](http://news.ahwang.cn/life/20260326/2986554.html)：首次将 Agentic AI 引入前道晶圆厂，缺陷根因定位 **38h → 5.4min**，获 SEMICON China 2026 良率提升奖
- [智现未来 FabSyn-YES](https://www.semi.org.cn/site/semi/article/cef674f0792b424898e0b8a2502d69ed.html)：良率溯因 **4h → 1min**，缺陷识别准确率 96%，良率预测准确率 90%+
- [MongoDB 参考架构](https://www.mongodb.com/company/blog/innovation/how-mongodb-atlas-powers-agentic-ai-for-semiconductor-yield-optimization)：LangGraph ReAct Agent + 4 工具（查告警/查 wafer/查时序参数/检索历史 RCA）
- [ThirdAI](https://www.ibselectronics.hk/resources/news/thirdai-raises-$3m-seed-to-deploy-causal-ai-for-faster-root-cause-analysis-in-semiconductor-fabs/)：因果智能层，RCA 时间降 80%
- [StackAI](https://www.stackai.com/blog/the-top-ai-agent-use-cases-for-semiconductor-manufacturing-in-2026)：跨 5+ 系统（MES/SPC/缺陷/量测/设备遥测）数据孤岛是根因分析的核心痛点
- Gartner：预计 2028 年前该领域自动化达 15%

### 为什么契合引擎

根因分析的本质 = 多源关联 + 多步推理 + 并行排查，与「主从并行子 Agent 编排」（task.md 阶段 B）完全同构。MongoDB 的参考架构正是 ReAct Agent + 多工具链逐层推理。

### 工程亮点（区别于通用 demo）

- **证据链**：每个结论附数据来源，可审计（对标 FabYield-PM 的 provenance 要求）
- **置信度分级**：区分确定性事实与模型推断
- **显式放弃**：证据不足时明确"不能下结论 + 建议补充什么数据"，而非硬编结论
- **人机协同**：Agent 提议根因与下一步，工程师验证，不做自主设备控制

### 落地方式（不依赖真实 fab）

用**可复现的模拟数据生成器**：合成 wafer map + 设备告警 + FDC 时序参数 + 历史案例库。Agent 的价值在编排与推理，数据可合成、可测试、可演示。

### 工具清单

下表维护工具的产品用途与数据来源；实现和测试状态统一见 [ALIGNMENT.md](../ALIGNMENT.md)，不在此重复登记。

| 工具 | 作用 | 数据源 |
| --- | --- | --- |
| `query_batch_yield` | 批次良率查询（按时间/机台/批次） | 模拟 YMS 数据 |
| `query_equipment_alerts` | 设备告警 / PM 记录 | 模拟 MES 数据 |
| `query_fdc_params` | FDC 工艺参数偏离检测 | 模拟 FDC 时序 |
| `query_defect_map` | wafer map 缺陷模式分析 | 模拟缺陷检测数据 |
| `search_historical_rca` | 历史案例检索 | 模拟案例库（关键词匹配，RAG 后续） |

### 按需求评估的扩展能力

- **embedding / 向量检索**：历史案例 RAG（`search_historical_rca`）
- **MemoryService**：沉淀历史排查经验（"Agent 永不遗忘过往 excursion"）

---

## 备选方向：工艺知识助手

> **状态**：第二项目备选。当前项目做良率 RCA 为主，此方向作为后续独立项目，记录以免遗忘。

- **一句话定位**：工业版 RAG——将工厂 SOP、机台维护日志、历史生产数据向量化封装为专属知识库，AI Agent 检索问答
- **行业参考**：[鼎华智能](https://digihua.com.tw/news260317/?num=21) 联合 Axe Innovation 的"工业版 RAG"——SMT 生产线 OEE 异常从数小时压缩至秒级
- **与引擎的关系**：关联弱（单 Agent 检索即可，无需多 Agent 编排），工程价值独立
- **若启动**：复用 embedding/向量能力、ContextManager、chat 路由；新增文档解析管线（PDF/手册/日志 → chunk → 向量）

---

## 已关闭方向：EDA 设计辅助

> **状态**：关闭（结构性不可落地，非"暂缓"）。记录原因，避免未来重新评估浪费精力。

四个硬约束：

1. **数据依赖**：需真实工艺 PDK（器件模型/库单元/设计规则）、lib 库、RTL/GDS/netlist —— 晶圆厂与设计公司的核心机密 IP，个人无法获取
2. **工具链依赖**：行业方案编排商业 EDA 工具（Synopsys DC/ICC、Cadence Virtuoso/Innovus、Siemens Questa/Calibre），按年授权、数十万至数百万美元、核心算法封闭；开源替代（OpenLane/Yosys）仅覆盖极小流程
3. **验证零容错**：芯片设计一个 DRC 违规/时序违例 = 流片失败。Agent 产出须经 signoff 级物理引擎验证（行业强调 self-verifying agents），无真实工具链则无法验证 Agent 价值
4. **领域知识门槛**：评估设计质量需器件物理、EDA 算法专业知识，工具跑通也无法判断结果好坏

**对比良率 RCA**：良率 RCA 绕开全部四条 —— 数据可合成、结果由工程师人工验证（幻觉被证据链+置信度约束）、零 license 依赖、领域门槛低。

---

## 里程碑与闭环

按「先出闭环、引擎后置」原则切分，每步均可演示：

以下任务接口是产品交互示例，具体 URL、流协议和状态字段在接口契约中统一确认，不能据此认定接口已经存在。

| 闭环 | 内容 | 依赖 |
| --- | --- | --- |
| **闭环 1** | 任务受理骨架：`POST /api/tasks` 提交 → worker 异步执行 → `GET /api/tasks/{id}` 查进度 → SSE 进度事件。先复用并核验 TaskService 的并发闸门能力；是否需要队列由受理/查询契约决定 | 核验现有并发与运行边界 |
| **闭环 2** | 编排：主 Agent 拆分 → 并行子 Agent → 汇总（task.md 阶段 B） | 闭环 1 |
| **闭环 3** | 优先级队列 + 背压（task.md 阶段 A 完整形态） | 闭环 2 |

### 验收标准

一个模拟的良率异常 case，系统能靠多 Agent 并行排查自动收敛到正确根因，输出带**证据链**的根因报告（每步结论可回溯到数据来源）。

---

## 工具模块产品导向取舍

> 工具模块（`app/integration/tools/`）的投入以本文产品主链路为依据；产品目标不能越过已确认的契约、授权和安全边界。

**定位**：工具执行、验证与结果审计服务 RCA 主链路。项目文档描述了 5 个带来源元数据的模拟 RCA 工具；它们不证明真实 fab 接入或完整多 Agent 产品闭环。selector / 审批骨架 / 热加载按下表限定投入，实现状态以 ALIGNMENT 为准。

### 能力评估

| 组件 / 能力 | 服务主链路 | 定位 |
| --- | --- | --- |
| registry / executor / validator / result_processor / auditor / error_code / 生命周期钩子 | ✅ 直接 | RCA 工具全都要，保留 |
| stats（执行统计） | 🔶 间接 | 场景可观测，保留 |
| selector（选择器占位） | ❌ 纯预留 | 工具 <10 全量注入足够，不扩展 |
| 审批骨架（ApprovalGate） | ❌ 未来人机协同 | 预留（产品亮点「人机协同」可能用） |
| 热加载（ExternalToolLoader） | ❌ 无当前场景驱动 | 引擎资产，不投入 |
| 内置 search / web_browse | 🔶 弱相关 | 备选方向（工艺知识助手）能力 |
| 内置 code_exec / readFile / writeFile | 🔶 间接有用 | RCA 分析跑脚本 / 写报告，保留 |

### 证据与投入边界

- **RCA 工具范围**：`query_batch_yield` 等 5 个模拟数据工具用于可重复演示；效果须由场景验收证明，不以文件存在或组件测试通过替代产品闭环
- **证据链契约**：RCA 工具结果 `metadata` 的 `source` / `query` / `timestamp` 应可回溯到输入数据与时间锚点；字段存在不等于结论已被证实
- **预留资产**：selector / 审批骨架 / 热加载明确「预留资产」定位，不新增引擎能力，除非主链路场景驱动
- **备选方向能力**：search / web_browse 标注为备选方向能力，主链路投入不放这里

### 取舍

热加载、审批骨架为「对齐工业蓝图」驱动，**无产品场景直接驱动**（本地单用户、固定工具）。既有能力是否保留取决于维护成本和风险，不能仅因已实现就认为无害；后续「工业蓝图对齐」类需求应**先用产品导向过滤**，而非直接落地。

---

## 相关文档

- [task.md](../application_doc/task_doc/task.md)（TaskService 调度枢纽 + 多 Agent 编排规划）
- [agent.md](../domain_doc/agent_doc/agent.md)（Agent 层：单任务执行）
- [llm.md](../integration_doc/llm_doc/llm.md)（LLM 服务层）
