...
> 依据 CLAUDE.md 第一硬性要求（产品导向），对工具模块（`app/integration/tools/`）当前状态的定位说明。

**结论**：工具模块的「引擎能力层」建设完备且直接服务主链路；**良率 RCA 场景工具已落地（5 个，带证据链元数据）**，产品价值已兑现。selector / 审批骨架 / 热加载保持预留定位。
...
- **RCA 工具已落地**：`query_batch_yield` 等 5 个工具（模拟数据源），复用已就绪引擎能力——工具模块产品价值兑现
- **证据链元数据已就绪**：RCA 工具结果 `metadata` 带数据来源 / 时间戳（`source` / `query` / `timestamp`），支撑「证据链」亮点
