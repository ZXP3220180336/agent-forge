...
> 依据：learning-agent 项目的 Planning 实验（Plan-then-Execute 基线 × 对照 ReAct × re-plan 故障修复）。此处只留工业级结论，实验细节见该学习项目。

1. **计划前置决策 + 执行被约束**：模型的"聪明"集中在规划阶段，执行阶段每步只做计划里的当前步骤（提示词强约束"不要做其他步骤"），不自由发挥——对比 ReAct 每轮现场决策。代价是计划质量决定执行成败。
2. **计划 = 可解释蓝图 + 依赖显式（depends_on）**：用户可见计划建立可信度（良率工程师能看到排查计划）；步骤间依赖显式标注、可审计——对比 ReAct 依赖隐含靠模型记忆。
3. **re-plan 处理执行期故障**：某步工具故障 / 外部依赖失败 → 把「失败详情 + 已执行结果」交给 LLM → 重新规划剩余步骤 → 继续执行。计划不是一次性的（对应上方「调整阶段」）。需限制重规划次数防死循环。
4. **re-plan 后依赖引用错位风险**：新计划 id 重新编号后，模型重规划时可能沿用旧计划的 `depends_on` 引用旧 id（实验实测：新计划 id 5-8 引用旧 id 1/2/3）。需重映射依赖，或执行时按依赖严格校验，否则长任务依赖链静默断裂。
5. **工具注册表强约束**：函数调用模式下模型只调用注册表（TOOLS）里的工具——计划 / 描述无法让它调用未注册工具（实测：注入不存在的工具名被模型忽略，改调可用工具）。这与工具模块的三层语义信号（[tools.md](../../integration_doc/tools_doc/tools.md)）互补：schema 质量影响选择，注册表决定可用集。
