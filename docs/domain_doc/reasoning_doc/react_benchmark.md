...
- [ReAct 策略工业级对标（完成度基准）](#react-策略工业级对标完成度基准)
  ...
  - [工业级参照：ReAct 工具循环能力清单](#工业级参照react-工具循环能力清单)
    ...
    - [增强项（13 项）—— 锦上添花或特定场景（HITL / 多 Agent / 长任务）才需要](#增强项13-项-锦上添花或特定场景hitl--多-agent--长任务才需要)
  - [本项目逐项对照](#本项目逐项对照)
    - [核心必备对照](#核心必备对照)
    - [增强项对照](#增强项对照)
...
- **三个增强级观察点**（见「发现的问题」）：循环停滞检测（#24）、空输出重试上限（#25）、refusal 显式处理（#26）未实现——均有既有护栏兜底或影响有限，不损害当前主链路。
...
### 增强项（13 项）—— 锦上添花或特定场景（HITL / 多 Agent / 长任务）才需要
...
| 24 | 循环停滞检测 | 同工具 + 同参数连续 N 轮判死循环终止（`same_action_llm_turn_limit`） | SMOL |
| 25 | 空输出重试上限 | 无效输出重试有限次后终止（防空转烧 token） | OpenAI |
| 26 | refusal 显式处理 | 模型拒答独立错误类型，短路不重试 | OpenAI |
...
| 24 | 循环停滞检测 | ❌ | 无停滞检测；靠 max_iterations / 总时长 / 成本上限三层兜底（死循环兜底前烧预算） |
| 25 | 空输出重试上限 | ❌ | `_handle_empty_output` 默认 CONTINUE 无限重试，仅靠 max_iterations 终结；默认配置下模型持续空回空转最多 10 轮、每轮发一次付费 LLM 调用 |
| 26 | refusal 显式处理 | ❌ | `StreamResult.refusal` 主循环未消费；拒答落在 stop + 非空 content 会被当作正常答案（DeepSeek 场景少见） |

---

## 发现的问题（按优先级）

> 均为**增强级观察点**（非核心必备缺失）——核心必备 13 项全部完成，下列三项不损害当前主链路（当前单用户本地 + 引擎主链路场景），按「不做或预留」原则延后（见产品导向视角）。

| 优先级 | 观察点 | 现状 | 兜底 | 补强价值 |
| --- | --- | --- | --- | --- |
| 高 | 空输出重试上限（#25） | `_handle_empty_output` 默认 CONTINUE 无限重试 | max_iterations 终结 | 改一行默认重试 2 次即止，防模型持续空回时空转烧钱（默认无成本上限时最明显） |
| 中 | 循环停滞检测（#24） | 无同工具重复判定 | max_iterations / 总时长 / 成本上限三层 | 防模型「反复调同一工具同一参数」死循环时兜底前烧预算；成本上限开启后优先级下降 |
| 低 | refusal 显式处理（#26） | `StreamResult.refusal` 主循环未消费 | 无（会当正常答案返回） | 拒答短路不重试、不误判成功；DeepSeek 场景少见，等真实触发 |
