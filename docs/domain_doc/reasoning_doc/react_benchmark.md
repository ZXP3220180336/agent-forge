...
- **两个增强级观察点**（见「发现的问题」）：循环停滞检测（#24）、refusal 显式处理（#26）未实现——均有既有护栏兜底或影响有限，不损害当前主链路。
...
> 均为**增强级观察点**（非核心必备缺失）——核心必备 13 项全部完成，下列两项不损害当前主链路（当前单用户本地 + 引擎主链路场景），按「不做或预留」原则延后（见产品导向视角）。
