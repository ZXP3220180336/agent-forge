...
`app/models/` 是系统的**数据模型层**，承载所有结构化数据的定义，职责分两部分：
...
**文件**：`app/models/database/base.py`（8 行）
...
**文件**：`app/models/database/session.py`（23 行），表名 `sessions`
...
**文件**：`app/models/database/messages.py`（30 行），表名 `messages`
...
当前唯一的模型使用方是 `SessionManager`（`app/services/session_manager.py`），它把 **Redis 热缓存 + SQLAlchemy 持久化**组合使用。关键使用点：
...
任务表用于持久化 **Agent 任务的执行记录**。内存态任务结构已定义在 `app/services/task_service.py` 对应的 `Task` 数据结构中（见 [task.md](../service_doc/task_doc/task.md)「数据模型」小节），含 `task_id` / `user_request` / `priority` / `status` / `parent_task_id` / `sub_tasks` / `agent_result` / `error` / `created_at` / `started_at` / `completed_at` 等字段。
...
工具调用日志表用于持久化**每次工具调用的审计记录**。工具系统的统一抽象见 [tools.md](../tool_doc/tools.md)（`BaseTool` / `ToolResult`，5 个内置工具）。
...
**解决**：统一到 `app/models/database/base.py` 中声明**唯一** `Base`，所有模型 `from .base import Base` 继承。任何新增模型（如未来的 `task` / `tool_log`）都必须复用该 `Base`，**严禁**在自己文件里再写 `declarative_base()`。
...
- 对外导出走 `app/models/__init__.py` 与 `app/models/database/__init__.py`
...
3. 依据 `tool_doc/tools.md` 落地 `tool_log.py` 工具调用日志表
...
3. 在 `app/models/database/__init__.py` 中导出（若对外使用还需在 `app/models/__init__.py` 导出）
