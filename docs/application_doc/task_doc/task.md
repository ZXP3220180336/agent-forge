...
> 依据：learning-agent 项目的多 Agent（Orchestrator-Worker）实验（拆分 × 并行 × 聚合，含依赖陷阱 / 合并传染的实测与修复）。此处只留工业级结论，实验细节见该学习项目。

1. **子任务必须自包含 + 依赖要传递**：若产生 `depends_on`，需把前序子结果注入后续子 Agent——否则子 Agent 隔离拿不到，会**冗余重算**（实测：无依赖传递时两个子 Agent 重复计算同一转换）。
2. **"汇总"不是子任务**：拆出"依赖全部前置的汇总子任务"会让合并 / 调度逻辑**传染性塌缩**（Aggregation Contagion）——所有独立子任务被吞并成 1 个巨型子任务，并行性归零。聚合是主 Agent 的职责，独立于子任务调度。
3. **提示词约束 + 程序校验双保险**：主 Agent 拆分不稳定（同一任务多次拆出不同形态，甚至输出 schema 本身）——程序侧需依赖检测 + 合并护栏兜底，不能只靠提示词。
4. **拆分质量决定并行价值**：拆不好（巨型子任务）并行性归零；合并描述若引用"子任务X"还会导致子 Agent 算错。拆分产物应是自包含的计算步骤。
