...
> **更新日期**：2026-08-04
> **文档定位**：工具层 `app/tools/builtin/` 子模块 —— 内置工具的定义、自动发现机制与各工具实现详解。
...
`builtin` 是工具层 `app/tools/` 下的**内置工具子模块**，存放随系统发布、开箱即用的工具实现，与预留的 `external` 子模块（第三方工具，按需加载）互为补充。
...
`BaseTool`（`app/tools/base.py`，129 行）是所有工具的抽象基类，定义了 4 个必须实现的抽象成员和 3 个通用具体方法。
...
`ToolResult`（`app/tools/base.py`）是工具执行的统一返回结构，`BaseTool.execute()` 必须返回它：
...
**文件**：`app/tools/builtin/search.py`（104 行）｜**名称**：`search`
...
**文件**：`app/tools/builtin/file_ops.py`（111 行）｜**名称**：`readFile`
...
**文件**：`app/tools/builtin/file_ops.py`｜**名称**：`writeFile`
...
**文件**：`app/tools/builtin/code_exec.py`（138 行）｜**名称**：`code_exec`
...
**文件**：`app/tools/builtin/web_browse.py`（272 行）｜**名称**：`web_browse`
...
`app/tools/external/__init__.py`（0 字节空文件）为**第三方/外部工具预留**位置。
